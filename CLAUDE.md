...
- **v1.3.0 (Build 10) built and submitted to App Store Connect 2026-08-09.** EAS build `cc060d02-9f23-481d-b9cd-e6a808b59b62`, submission `9d858200-0236-4163-a2fe-4cfbbcbafecb`. Confirmed via `eas submit` output: App Version 1.3.0, Build number 10 (continuing EAS's own sequence after Build 9, as expected). Uploaded successfully — Apple was still processing it as of submission time (~5-10 min typical). Not yet visible in TestFlight/App Review at doc-update time — check https://appstoreconnect.apple.com/apps/6761027461/testflight/ios. Last manual step before review: App Privacy form + What's New text — see Open Items.

## Open items (as of 2026-08-09)

- **Only remaining step for v1.3.0: the App Store Connect App Privacy form + What's New text + submit for review — manual, in the App Store Connect web UI, once Build 10 finishes Apple's processing.** Declare "Product Interaction" under Usage Data, purpose "Analytics", user identification "No", tracking "No" (per Aptabase's own submission guide). What's New text: "Added an About screen with app version info. Refreshed the scan animation." — plain, factual, matches the style already used in the live v1.2.0 listing.
- **All device verification for v1.3.0 confirmed by Lee (2026-08-09):** About screen reads version/build correctly; scan-loop audio plays from the start with no delay; all 5 Aptabase events fired live in one session (`App_Opened`, `Screenshot_Uploaded`, `Analysis_Complete` x20+, `Paywall_Viewed`, `Purchase_Completed` x2 — `tier: pro_monthly`, `period_type: NORMAL`); App Version reported correctly (no more `1.0.0` anomaly).
...
- **Committed and pushed to GitHub 2026-08-09**, commits `a29c8ce` and `f02c7ac` — includes the SBM-03 through SBM-06 handoffs. `npm install` confirmed run (package-lock.json changed).
